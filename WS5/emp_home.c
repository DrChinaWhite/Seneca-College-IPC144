...
int main(void)
{
	...
	do
	{
		...
		// Additiononal options to update employee salary or remove employee
		// @AT-HOME
		...
		case 1: // Display Employee Data
			...
			for (i = 0; i < SIZE; i++) {
				...
				if (emp[i].empNumber > 0) {
					...
					printf("%6d%9d%11.2lf\n", emp[i].empNumber, emp[i].empAge, emp[i].empSalary);
				}
			}
		...
		case 3: // Update Employee Salary
			...
			do
			{
				...
				// scan the array to see if there is an employee number that matches
				for (i = 0; i < SIZE; i++) {

					if (idInput == emp[i].empNumber) {
			...
			// overwrite the employee salary
			printf("The current salary is %.2f\n", emp[idFound].empSalary);
			printf("Enter Employee New Salary: ");
			scanf("%lf", &emp[idFound].empSalary);
			printf("\n");
